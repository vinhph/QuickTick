...
GO
PRINT N'Starting rebuilding table [dbo].[ecfVersionProperty]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_ecfVersionProperty] (
    [WorkId]        INT              NOT NULL,
    [ObjectId]      INT              NOT NULL,
    [ObjectTypeId]  INT              NOT NULL,
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PK_ecfVersionProperty] PRIMARY KEY CLUSTERED ([WorkId] ASC, [MetaFieldId] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[ecfVersionProperty])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_ecfVersionProperty] ([WorkId], [MetaFieldId], [ObjectId], [ObjectTypeId], [MetaClassId], [MetaFieldName], [LanguageName], [Boolean], [Number], [FloatNumber], [Money], [Decimal], [Date], [Binary], [String], [LongString], [Guid])
        SELECT   [WorkId],
                 [MetaFieldId],
                 [ObjectId],
                 [ObjectTypeId],
                 ...
                 [Guid]
        FROM     [dbo].[ecfVersionProperty]
        ORDER BY [WorkId] ASC, [MetaFieldId] ASC;
    END

DROP TABLE [dbo].[ecfVersionProperty];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_ecfVersionProperty]', N'ecfVersionProperty';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_ecfVersionProperty]', N'PK_ecfVersionProperty', N'OBJECT';
...
GO
PRINT N'Starting rebuilding table [dbo].[LineItemDiscount]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_LineItemDiscount] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_IX_LineItemDiscount] UNIQUE NONCLUSTERED ([DiscountId] ASC, [LineItemId] ASC),
    CONSTRAINT [tmp_ms_xx_constraint_PK_LineItemDiscount] PRIMARY KEY NONCLUSTERED ([LineItemDiscountId] ASC)
);

CREATE CLUSTERED INDEX [tmp_ms_xx_index_IX_LineItem_OrderGroupId]
    ON [dbo].[tmp_ms_xx_LineItemDiscount]([OrderGroupId] ASC);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[LineItemDiscount])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_LineItemDiscount] ON;
        INSERT INTO [dbo].[tmp_ms_xx_LineItemDiscount] ([OrderGroupId], [LineItemDiscountId], [LineItemId], [DiscountId], [DiscountAmount], [DiscountCode], [DiscountName], [DisplayMessage], [DiscountValue])
        SELECT   [OrderGroupId],
                 [LineItemDiscountId],
                 [LineItemId],
                 [DiscountId],
                 ...
                 [DiscountValue]
        FROM     [dbo].[LineItemDiscount]
        ORDER BY [OrderGroupId] ASC;
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_LineItemDiscount] OFF;
    END

DROP TABLE [dbo].[LineItemDiscount];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_LineItemDiscount]', N'LineItemDiscount';

EXECUTE sp_rename N'[dbo].[LineItemDiscount].[tmp_ms_xx_index_IX_LineItem_OrderGroupId]', N'IX_LineItem_OrderGroupId', N'INDEX';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_IX_LineItemDiscount]', N'IX_LineItemDiscount', N'OBJECT';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_LineItemDiscount]', N'PK_LineItemDiscount', N'OBJECT';
...
GO
PRINT N'Starting rebuilding table [dbo].[OrderGroupNote]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_OrderGroupNote] (
    ...
    [Created]      DATETIME         NOT NULL,
    [LineItemId]   INT              NULL,
    CONSTRAINT [tmp_ms_xx_constraint_PK_OrderGroupNote] PRIMARY KEY NONCLUSTERED ([OrderNoteId] ASC)
);

CREATE CLUSTERED INDEX [tmp_ms_xx_index_IX_OrderGroupNote_OrderGroupId]
    ON [dbo].[tmp_ms_xx_OrderGroupNote]([OrderGroupId] ASC);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[OrderGroupNote])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_OrderGroupNote] ON;
        INSERT INTO [dbo].[tmp_ms_xx_OrderGroupNote] ([OrderGroupId], [OrderNoteId], [CustomerId], [Title], [Type], [Detail], [Created], [LineItemId])
        SELECT   [OrderGroupId],
                 [OrderNoteId],
                 [CustomerId],
                 [Title],
        ...
        FROM     [dbo].[OrderGroupNote]
        ORDER BY [OrderGroupId] ASC;
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_OrderGroupNote] OFF;
    END

DROP TABLE [dbo].[OrderGroupNote];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_OrderGroupNote]', N'OrderGroupNote';

EXECUTE sp_rename N'[dbo].[OrderGroupNote].[tmp_ms_xx_index_IX_OrderGroupNote_OrderGroupId]', N'IX_OrderGroupNote_OrderGroupId', N'INDEX';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_OrderGroupNote]', N'PK_OrderGroupNote', N'OBJECT';
