...
Function UpdateMediachaseSearchConfig($configPath)
{
    $config = Get-Config $configPath

    $providers = $config.SelectSingleNode("/Mediachase.Search/SearchProviders/providers")

    $element = $config.SelectSingleNode("/Mediachase.Search/SearchProviders/providers/add[@name='FindSearchProvider']")
    if ($element -eq $null)
    {
        $element = $config.CreateElement("add", $null)
        $providers.AppendChild($element) | Out-Null

        $element.SetAttribute('name','FindSearchProvider')
        $element.SetAttribute('type','EPiServer.Commerce.FindSearchProvider.FindSearchProvider, EPiServer.Commerce.FindSearchProvider')

        $element.SetAttribute('serviceUrl','http://find.episerver.com')
        $element.SetAttribute('defaultIndex','{defaultIndex}')   
    }    
...
#update FindSearchProvider configurations
$projectFile = Get-Item $project.FullName
$mediachaseSearchConfigPath = join-path $projectFile.Directory.FullName "Configs\Mediachase.Search.Config"
UpdateMediachaseSearchConfig -configPath $mediachaseSearchConfigPath

# Update webconfig if not exist
...
if (Test-Path $webConfigPath) 
{
	...
	if($webConfig.configuration.'episerver.find' -eq $null)
	{
		$episerverfind = $webConfig.CreateElement("episerver.find")
        $episerverfind.SetAttribute("serviceUrl", "http://find.episerver.com")
		$episerverfind.SetAttribute("defaultIndex","{defaultIndex}")
		$webConfig.DocumentElement.AppendChild($episerverfind) | Out-Null
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUFGPs4O61O90PeTmbYGuwBi4P
# JLCgghLKMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# BBRvBu8epxu/IU7gSGH68fyVF1WTwjANBgkqhkiG9w0BAQEFAASCAQCGaBZDSExf
# TIe1OTNAGE6C7OWpl/UHzbbgMzKwMCQ1wt7I2iWM3Ffl2Elhng/zDZoBhCzfHbEJ
# Mp+j265brg2YuIJBtJCwJW1mRVv2fLmFp6YqoPla8wovRTQ63KQJtFiCMXG5zVMl
# fvkMZpjoHyObRugZqXaU+H60MIHbtLuWITyYrHoUJHzmnZk3nx2dcQ2juGgsYXl3
# N6C6DC9r9Kxf+Cq/Ntk3CzvZHGLQQi03eHBhGjHkotXAZdD7shFwGfJE2fKNFain
# nHLZWVf020zTcLq7yJ7Q5HkcfI/rdsqSFm6ma4/nd/wBlkvxLAW8+1Lp5JXBo3XT
# 3nw/sajlsSiaoYICCzCCAgcGCSqGSIb3DQEJBjGCAfgwggH0AgEBMHIwXjELMAkG
...
# BwEwHAYJKoZIhvcNAQkFMQ8XDTE4MDUyNTExMTk0NlowIwYJKoZIhvcNAQkEMRYE
# FDyMrHJGZE7AxjItv7Cigsvz48uzMA0GCSqGSIb3DQEBAQUABIIBAFibw5y07Px8
# Hmr0BBp3nG4WMjGvIK3k34A35FbLvI3pNj0Hoq52KFgzfpz7dxVU8jok2mX3cKVM
# Rl7lK2Rm9rjo0yY2yNnki+9mDt/Yy8G//Lc1ryaNRTxeV9eHfnGImzA0zjNBAEfI
# 5y7j2KbGvbnVTrx9IXCcriiIEcY9QCt9fMKlDJmPop/gIYuzaod3iQhUmrtHYVBM
# FjeNR383yLodmOqd8zdMRdOAWJRDUYQ5moaYf/zvg/1CgcItj74FBjG8TLRSE1gi
# 0yG9uIo8pnXCrzYYiyO2mXE9z5mqB31UucG8HqCmMfMmnKp7U4mWx2md+cmcsNfv
# qe/GQo6hlPc=
